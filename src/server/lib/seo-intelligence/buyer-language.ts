...
export type BuyerLanguageSignalKind = "PAIN" | "TASK" | "DESIRED_OUTCOME" | "CONSTRAINT" | "FRUSTRATION" | "COMPARISON_NEED" | "PURCHASE_CONSIDERATION";
export type BuyerLanguageSourceClass = "SITE_CAPTURED" | "USER_PROVIDED" | "MODEL_INFERRED" | "UNTRACEABLE";
...
export type BuyerLanguageObservation = {
  ...
  claimKey: string;
  stance: BuyerLanguageStance;
  evidence: { family: "BUYER_LANGUAGE"; rawArtifactRef: string; sha256: string; captureId: string; sourceObservationId: string };
...
function acceptedSiteSource(source: BuyerLanguageRawSource): boolean {
  return source.sourceClass === "SITE_CAPTURED" && Boolean(source.sourceId) && Boolean(source.sourceUrl) && /^https:\/\//.test(source.sourceUrl ?? "") && Boolean(source.text.trim()) && Boolean(source.claimKey.trim());
}

function unknownFor(source: BuyerLanguageRawSource): BuyerLanguageUnknown {
  return {
    topic: "BUYER_LANGUAGE_SOURCE",
    reason: source.sourceClass === "SITE_CAPTURED" ? "PROVENANCE_FAILURE" : "SOURCE_REJECTED",
    sourceId: source.sourceId,
    nextAction: "COLLECT_BUYER_LANGUAGE",
  };
}

function contradictions(observations: BuyerLanguageObservation[]): BuyerLanguageContradiction[] {
  ...
  return [...byClaim.entries()].flatMap(([claimKey, values]) => {
    ...
    return supportingObservationIds.length && conflictingObservationIds.length ? [{ claimKey, supportingObservationIds, conflictingObservationIds, status: "OPEN" as const, nextAction: "COLLECT_BUYER_LANGUAGE" as const }] : [];
  });
}

/** Deterministic, injected-source vertical slice. It does not fetch or score. */
export async function processBuyerLanguageSources(sources: BuyerLanguageRawSource[], store: ImmutableEvidenceStore): Promise<BuyerLanguageSliceResult> {
  ...
  for (const source of sources) {
    if (!acceptedSiteSource(source)) {
      ...
      admissions.push({ sourceId: source.sourceId, outcome: source.sourceClass === "SITE_CAPTURED" ? "UNKNOWN_INSUFFICIENT_PROVENANCE" : "REJECTED" });
      continue;
    }
    let artifact;
    try {
      artifact = await store.captureArtifact({ family: "BUYER_LANGUAGE", provider: source.provider, requestMetadata: { sourceClass: source.sourceClass, sourceId: source.sourceId, sourceUrl: source.sourceUrl, claimKey: source.claimKey }, capturedAt: source.capturedAt, status: 200, bytes: new TextEncoder().encode(source.text), providerVersion: null, adapterVersion: "phase-3b", contentType: "text/plain" });
    } catch {
      unknowns.push({ topic: "BUYER_LANGUAGE_SOURCE", reason: "PROVENANCE_FAILURE", sourceId: source.sourceId, nextAction: "COLLECT_BUYER_LANGUAGE" });
      admissions.push({ sourceId: source.sourceId, outcome: "UNKNOWN_INSUFFICIENT_PROVENANCE" });
      continue;
    }
    observations.push({ id: `buyer-language:${artifact.sha256}`, text: source.text, signalKind: source.signalKind, speakerOrContext: source.sourceUrl!, directlyObserved: true, category: "SITE_OBSERVED", evidenceFunction: "BUYER_LANGUAGE", claimKey: source.claimKey, stance: source.stance, evidence: { family: "BUYER_LANGUAGE", rawArtifactRef: artifact.ref, sha256: artifact.sha256, captureId: artifact.ref, sourceObservationId: source.sourceId! }, limits: "OBSERVED_PHRASE_ONLY" });
...
export function replayBuyerLanguageSlice(result: BuyerLanguageSliceResult): BuyerLanguageSliceResult {
